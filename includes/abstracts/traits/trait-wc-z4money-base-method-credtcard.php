...
if ( ! class_exists( 'Wc_Z4Money_Base_Method_CredtCard' ) ) {
 ...
 */
    trait Wc_Z4Money_Base_Method_CredtCard
    {
		...
		{			
			...
			for ( $i = 1; $i <= $installments; $i++ ) {
				...
				$credit_interest = sprintf( __( 'no interest. Total: %s', 'wc-z4money' ), sanitize_text_field( wc_price( $order_total ) ) );
				...
				if ( 'client' == $this->installment_type && $i >= $this->interest && 0 < $interest_rate ) {
					...
					if ( $credit_total < $interest_total ) {
						...
						$credit_interest = sprintf( __( 'with interest of %s%% a.m. Total: %s', 'wc-z4money' ), $this->get_valid_value( $this->interest_rate ), sanitize_text_field( wc_price( $interest_order_total ) ) );
					}
				}
				...
				$at_sight = ( 1 == $i ) ? 'z4money-at-sight' : '';

				$html .= '<option value="' . $i . '" class="' . $at_sight . '">' . sprintf( __( '%sx of %s %s', 'wc-z4money' ), $i, sanitize_text_field( wc_price( $credit_total ) ), $credit_interest ) . '</option>';
			}
		...
		public function get_accepted_brands_list( $methods ) 
		{
			...
			foreach ( $methods as $method ) {
				...
				if ( 1 == $total ) {
					...
					$list .= sprintf( __( 'and %s', 'wc-z4money' ), $name );
				} else {
					$list .= $name . ', ';
				}
		...
		protected function validate_installments( $posted, $order_total ) 
		{
			...
			try {
				...
				if ( ! isset( $posted[STATIC::CARD_INSTALLMENTS] ) || '' === $posted[STATIC::CARD_INSTALLMENTS] ) {
					throw new Exception( __( 'Please select a number of installments.', 'wc-z4money' ) );
				}
				...
				if ( $installments > $_installments || 1 != $installments && $installment_total < $smallest_value ) {
					 throw new Exception( __( 'Invalid number of installments!', 'wc-z4money' ) );
				}
			} catch ( Exception $e ) {
		...
		public function get_creditcard_data( $posted, $order ) 
		{
			...
			if ( ! $this->validate_installments( $posted, $order->order_total ) ) {
				throw new Exception( __( 'Number or value of installments is invalid', 'wc-z4money' ) );
			}
			...
			if ( ! isset( $avalible_brands[$card_brand] ) ) {
				throw new Exception( __( 'Payment has not been made. Your card brand is not supported', 'wc-z4money' ) );
			}
			...
			update_post_meta( $order->id, '_wc_z4money_card', $data );
