...
if ( ! class_exists( 'Wc_Z4Money_Logger' ) ) {
 ...
 */
	class Wc_Z4Money_Logger 
	{
		...
		const WC_LOG_FILENAME = 'wc-z4money';
		...
		public function add( $message, $start_time = null, $end_time = null ) 
		{
			...
			if ( apply_filters( 'wc_z4money_logging', true, $message ) ) 
			{
				if ( empty( $this->logger ) ) {
					
					if ( Wc_Z4Money_Helper::is_woocommerce_lt( '3.0' ) ) {
				...
				if ( ! is_null( $start_time ) ) {
					...
					$log_entry  = "\n" . '====' . WC_Z4MONEY_NAME . ' Version: ' . WC_Z4MONEY_VERSION . '====' . "\n";
